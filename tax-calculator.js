...
(function () {
  ...
  /* ============================ CONFIG ============================ */
  // Cloudflare Worker that forwards submissions to Freshdesk.
  // Replace with your deployed Worker URL.
  var WORKER_URL = 'https://tax-calc-freshdesk.YOUR-ACCOUNT.workers.dev/';
  ...
  function buildWorkerPayload(name, email, r) {
    var occObj = OCCUPATIONS.find(function (o) { return o.key === r.occupation; });
    var langLabel = { en: 'English', es: 'Espanol', pt: 'Portugues' }[lang] || lang;
    return {
      name: name,
      email: email,
      language: langLabel,
      taxYear: r.year,
      residency: t('res_' + r.residency),
      occupation: occObj ? occObj.label[lang] : r.occupation,
      wages: fmt(r.wages),
      interest: fmt(r.interest),
      businessIncome: fmt(r.business),
      rentalIncome: fmt(r.rental),
      totalIncome: fmt(r.totalIncome),
      deductionsBreakdown: deductionsBreakdown(),
      totalDeductions: fmt(r.totalDeductions),
      taxableIncome: fmt(r.taxableIncome),
      grossTax: fmt(r.tax),
      lito: fmt(r.lito),
      medicareLevy: fmt(r.ml),
      mls: fmt(r.mls),
      helpRepayment: fmt(r.help),
      totalTax: fmt(r.total),
      paygw: fmt(r.paygw),
      paygInstalments: fmt(r.instalments),
      totalPrepaid: fmt(r.totalPrepaid),
      outcomeType: r.outcome >= 0 ? 'Refund' : 'Payable',
      estimatedOutcome: fmt(Math.abs(r.outcome)),
      netIncome: fmt(r.net),
      effectiveRate: fmtPct(r.eff)
    };
  }
  ...
  function handleSubmit(withRedirect) {
    ...
    if (!WORKER_URL || WORKER_URL.indexOf('YOUR-ACCOUNT') !== -1) {
      setMsg('Form endpoint not configured. Please contact info@taxbne.com.au.', 'error');
      return;
    }

    var r = calculate();
    var payload = buildWorkerPayload(name, email, r);
